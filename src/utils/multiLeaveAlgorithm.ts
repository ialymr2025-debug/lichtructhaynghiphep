...
export function buildMultiLeaveResults(leaves: Leave[], chucDanh: string, staffData: string[][]) {
  ...
  const reliefTracker: Record<number, { C: number, K: number, N: number, totalC: number, totalN: number, totalK: number, swapped: Set<string>, lastOTronDate?: number }> = {};
  ...
  while ((dateKey = getNextUnprocessed()) !== null) {
    ...
    activeLeaves.forEach(l => {
      ...
      if (!reliefTracker[absentKip]) {
        // Pre-calculate total counts for this leave
        let tC = 0, tN = 0, tK = 0;
        let curr = new Date(l.start);
        while (curr <= l.end) {
          const sType = xacDinhCa(curr, absentKip);
          if (sType === 'C') tC++;
          if (sType === 'N') tN++;
          if (sType === 'K') tK++;
          curr.setDate(curr.getDate() + 1);
        }
        reliefTracker[absentKip] = { C: 0, K: 0, N: 0, totalC: tC, totalN: tN, totalK: tK, swapped: new Set() };
      }
      
      const s = xacDinhCa(ngay, absentKip);
      const yesterday = new Date(ngay.getTime() - 86400000);

      if (s === 'O') {
        // Helper Kip must be in "O tròn" (O after K)
        const helperKip = [1, 2, 3, 4, 5].find(k => {
          const curS = xacDinhCa(ngay, k);
          const preS = xacDinhCa(yesterday, k);
          return k !== absentKip && 
                 k !== RULES[absentKip].N.k && 
                 k !== RULES[absentKip].C.k && 
                 k !== RULES[absentKip].K.k &&
                 curS === 'O' && preS === 'K';
        });

        if (helperKip) {
          const lastOTron = reliefTracker[absentKip].lastOTronDate;
          const daysSinceLast = lastOTron ? Math.floor((ngay.getTime() - lastOTron) / 86400000) : 999;

          if (daysSinceLast > 3) {
            let targetShiftType = '';
            const nextS = xacDinhCa(tomorrow, absentKip);

            const heavier = reliefTracker[absentKip].totalN >= reliefTracker[absentKip].totalC ? 'N' : 'C';
            const lighter = reliefTracker[absentKip].totalN >= reliefTracker[absentKip].totalC ? 'C' : 'N';
            const cycleIdx = reliefTracker[absentKip].swapped.size % 3;

            if (cycleIdx === 0) {
              // 1st, 4th, 7th...: Support K (or follow tomorrow's shift if 1st and it's 2nd occurrence)
              if (reliefTracker[absentKip].swapped.size === 0) {
                if (nextS !== 'O' && reliefTracker[absentKip][nextS] === 1) {
                  targetShiftType = nextS;
                }
              } else {
                targetShiftType = 'K';
              }
            } else if (cycleIdx === 1) {
              // 2nd, 5th, 8th...: Support heavier shift
              targetShiftType = heavier;
            } else {
              // 3rd, 6th, 9th...: Support lighter shift
              targetShiftType = lighter;
            }

            if (targetShiftType) {
              const covererKip = RULES[absentKip][targetShiftType].k;
              const naturalShiftOfCoverer = xacDinhCa(ngay, covererKip);
              
              if (naturalShiftOfCoverer !== 'O' && !forcedAssignments[helperKip] && !forcedAssignments[covererKip]) {
                forcedAssignments[helperKip] = naturalShiftOfCoverer;
                forcedAssignments[covererKip] = 'O';
                forcedReliefs.push({ absentKip, shift: naturalShiftOfCoverer, helperKip });
                reliefTracker[absentKip].swapped.add(targetShiftType + '_' + reliefTracker[absentKip].swapped.size);
                reliefTracker[absentKip].lastOTronDate = ngay.getTime();
              }
            }
          }
        }
      ...
      if (s === 'K') reliefTracker[absentKip].K++;
          ...
          if (absentSet[absentKip]) {
            ...
            if (idx !== undefined) {
              ...
              results[idx].ketQua.push({
                ...
                isConflict: isConf,
                conflictNote: reliefInfo ? `⇄ Đổi ca cân bằng: Kíp ${k} trực thay cho Kíp ${reliefInfo.helperKip === k ? 'gốc' : reliefInfo.absentKip} (Kíp hỗ trợ)` : noteConf,
            ...
            extraRows.push({
              ...
              isConflict: isConf, 
              conflictNote: isConf ? noteConf : `△ Điều chỉnh hệ thống: ${timThay(k, chucDanh, staffData)} thay cho ${timThay(absentKip, chucDanh, staffData)}`,
